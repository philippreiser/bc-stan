...
mod_step1 <- cmdstan_model("GP_step_1_power_scaling.stan")
...
mcmc_trace(
  ...
  regex_pars = c("rho", "alpha", "sigma", "f")
)
fit_step1$summary()
...
mod_step2 <- cmdstan_model("GP_step_2_power_scaling.stan")
...
plot_pp <- function(fit,
  ...
  p <- ggplot() +
    ...
    geom_point(data = field_data, aes(x = x_plot, y = y_obs), color = "blue", size = 1) +
    theme_minimal() +
...
plot_pp(
  ...
  ylims    = c(-0.5, 12),
  title    = latex2exp::TeX(
    sprintf("GP Step 2 Mean of Posterior Predictive ($\\alpha_{real}=%.2f,\\;\\alpha_{sim}=%.2f$)",
            stan_data$alpha_real, stan_data$alpha_sim))
)
